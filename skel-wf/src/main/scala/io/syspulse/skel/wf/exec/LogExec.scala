...
class LogExec(wid:Workflowing.ID,name:String,dataExec:Map[String,Any]) extends Executing(wid,name,dataExec) { 
  def logger(msg:String): (String) => Unit = dataExec.getOrElse("log.level","INFO") match {
  ...
  override def exec(in:Let.ID,data:ExecData):Try[ExecEvent] = {
    ...
    logger(s"LOGGING: > ${dataExec.getOrElse("sys","[]").toString.repeat(25)}: data.exec=${dataExec}: data.workflow=${dataWorkflow}: data=${data}")
