...
class WorkflowSpec extends AnyWordSpec with Matchers with WorkflowTestable {
  ...
  "Workflow" should {
    ...
    "Build workflow dynamically from DSL: F-1(LogExec())->F-1[in-0]()" in {
    ...
    "Build workflow dynamically from DSL: F-1(LogExec(v=1,v=2))->F-1[in-0]" in {
    ...
    "Build workflow dynamically from DSL: F-1[in-2](LogExec(v1=1,v2=2))[out-2]->F-1[in-2]" in {
      val wf1 = Workflow.assemble("wf-11","Workflow-11","F-1[in-2](LogExec(v1=1,v2=2))[out-2]->F-1[in-2]")
      
      wf1 shouldBe a[Success[_]]

      wf1.get.execs.size should === (1)
      wf1.get.execs(0) should === (Exec("F-1","io.syspulse.skel.wf.exec.LogExec",in = Seq(In("in-2")), out = Seq(Out("out-2")), data = Some(Map("v1"->"1","v2"->"2"))))
      wf1.get.links.size should === (1)
