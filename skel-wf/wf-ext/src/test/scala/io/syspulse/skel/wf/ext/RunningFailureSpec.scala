...
/** Engine whose run is RUNNING but has a retrying task: the EngineTemporal folds a pending
 *  activity's lastFailure into status=RUNNING_RETRY + meta("err"). This stub simulates that result. */
class FailureEngine extends Engine {
  ...
  def getRuntime(ns: Option[String], runtimeId: String): Future[Option[EngineWorkflow]] =
    Future.successful(Some(EngineWorkflow(
      id = "PoR-Wf", runtimeId = runtimeId, name = "PoR-Flow", status = WorkflowStatus.RUNNING_RETRY,
      namespace = "default", meta = Map("err" -> "ProofOfOwnership: boom"),
      activities = Seq(EngineActivity("a1", "ProofOfOwnership", EngineActivity.KIND_ACTIVITY, EngineStatus.RUNNING_RETRY, detail = Some("boom"))))))
...
class RunningFailureSpec extends AnyWordSpec with Matchers with ScalatestRouteTest with BeforeAndAfterAll with WfRouteTest {
  ...
  "GET /config/resolve/{id} for a RUNNING workflow with a retrying task" should {
    "map the WorkflowConfig to RUNNING_RETRY and expose the error in meta.err" in {
      ...
      Get(s"/config/resolve/$RID") ~~> routes.routes ~> check {
        ...
        c.status shouldBe WorkflowStatus.RUNNING_RETRY
        c.meta.flatMap(_.get("err")).map(_.toString) shouldBe Some("ProofOfOwnership: boom")
      }
      Await.result(store.getWConf(cfg.id), 5.seconds).status shouldBe WorkflowStatus.RUNNING_RETRY
    }
  }
}
