#!/bin/bash
# Get DetectorSchema by id, or list with optional paging
#   ./detector-schema-get.sh [id]
#   FROM=0 SIZE=10 ./detector-schema-get.sh
ID=${1:-}
FROM=${FROM:-0}
SIZE=${SIZE:-5}
...
if [[ -n "$ID" ]]; then
  URL="$SERVICE_URI/detector/schema/$ID"
else
  URL="$SERVICE_URI/detector/schema"
