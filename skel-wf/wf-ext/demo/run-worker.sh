...
# Usage: ./run-worker.sh [dynamic|static] [namespace]
#   default mode dynamic; static -> only WorkflowType $WORKFLOW_NAME
...
echo "worker: mode=${1:-${MODE:-dynamic}} ns=${2:-$TEMPORAL_NAMESPACE} queue=$TASK_QUEUE temporal=$TEMPORAL_TARGET wf-ext=$WF_EXT_URL"
