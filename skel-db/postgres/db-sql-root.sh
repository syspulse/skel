...
if [ "$ALL_DB" != "" ]; then
PGPASSWORD=$ROOT_PASS psql --host=$DB_HOST --port=$DB_PORT --username=$ROOT_USER $PSQL_VARS -f ${SQL_FILE}
else
PGPASSWORD=$ROOT_PASS psql --host=$DB_HOST --port=$DB_PORT --username=$ROOT_USER -d $DB_DATABASE $PSQL_VARS -f ${SQL_FILE}
fi
