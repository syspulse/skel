...
class CountryStoreDB extends StoreDB[Country]("db","country") with CountryStore {
  ...
  def create:Try[Long] = {
    ctx.executeAction(s"CREATE TABLE IF NOT EXISTS ${tableName} (id VARCHAR(36) PRIMARY KEY, name VARCHAR(255), short VARCHAR(255) );")
  ...
  override def deleteAll:Try[CountryStore] = {
    log.info(s"truncate: ")
    clear()
    Success(this)
  }
  ...
  def getByName(name:String):Option[Country] = {
    ...
    if(name.size==2) {
      ctx.run(query[Country]
        .filter(o => 
          o.short.toLowerCase == lift(name.toLowerCase())
        )) match {
        case h :: _ => Some(h)
        case Nil => None
      }
