...
object DashJson extends JsonCommon {
  ...
  //       icon = fields.get("icon").filter(_ != JsNull).map(_.convertTo[String])
  //     )
  //   }
  // }

  implicit val jf_dash = jsonFormat11(Dash)

  implicit val jf_dash_layout = jsonFormat10(DashLayout)
  implicit val jf_dashs = jsonFormat2(Dashs)
  implicit val jf_dash_create = jsonFormat7(DashCreateReq)  
  implicit val jf_dash_update = jsonFormat8(DashUpdateReq)
