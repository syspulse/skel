...
class JobStoreDir(engine:JobEngine,dir:String = "store/") extends StoreDir[Job,UUID](dir) with JobStore {
  val store = new JobStoreMem(engine)
  ...
  def +(name:String,script:String,conf:Seq[String],inputs:Seq[String]):Try[Job] = {
    store.+(name,script,conf,inputs).flatMap(j => super.+(j).map(_ => j))
  }
