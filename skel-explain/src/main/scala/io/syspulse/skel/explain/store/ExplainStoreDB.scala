...
class ExplainStoreDB(configuration: Configuration, dbConfigRef: String)
  ...
  def create: Try[Long] = {
    ...
    val tsvExprPostgres =
      """regexp_replace(coalesce(name, ''), '[^a-zA-Z0-9]+', ' ', 'g') || ' ' ||
        |regexp_replace(coalesce(description, ''), '[^a-zA-Z0-9]+', ' ', 'g')""".stripMargin
    ...
    val CREATE_TABLE_POSTGRES_SQL =
      s"""CREATE TABLE IF NOT EXISTS ${tableName} (
        ...
        ts0 BIGINT,
        ts BIGINT,
        ${colExplainTsv} tsvector GENERATED ALWAYS AS (
  ...
  def search(query: String, from: Option[Long] = None, size: Option[Long] = None): Future[ExplainStore.Page] = {
    ...
    getDbType match {
      case "postgres" =>
        ExplainStore.postgresPrefixTsQuery(q) match {
          case None => Future.successful(ExplainStore.Page(Seq.empty, 0))
          case Some(tsq) =>
            val fts = s"${colExplainTsv} @@ to_tsquery('simple', '${sqlLit(tsq)}')"
