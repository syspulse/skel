...
class ExplainRoutes(registry: ActorRef[Command])(implicit context: ActorContext[_], config: Config)
  ...
  private def canAccessOid(authn: Authenticated, oid: Option[String]): Boolean =
    ...
    ExtAuth.getOwner(authn) == oid

  // oid is not used for admin access
  private def canAccessAdmin(authn: Authenticated, oid: Option[String] = None): Boolean =
    Permissions.isAdmin(authn) ||
    Permissions.isService(authn)    
  ...
  @Operation(tags = Array("explain"), summary = "Run Explain Rule",
    ...
    requestBody = new RequestBody(content = Array(new Content(schema = new Schema(implementation = classOf[ExplainReq])))),
    responses = Array(
  ...
  def routeExplain(rid: Option[String]) = (get | post) {
    parameters("style".?, "oid".?) { (styleOpt, oidQuery) =>
      (entity(as[ExplainReq]) | provide(ExplainReq())) { req =>
        val style = styleOpt.orElse(req.style).getOrElse("")
  ...
  override def routes: Route = cors(corsAllow) {
    concat(
      ...
      pathEndOrSingleSlash {
        get {
          entity(as[ExplainReq]) { req =>
            parameters("style".?, "oid".?) { (styleOpt, oidQuery) =>
              authenticate()(authn => {
                authorize(canAccessAdmin(authn, oidQuery)) {
        ...
        delete {
          parameters("oid".?) { oidQuery =>
            authenticate()(authn => {              
              authorize(canAccessAdmin(authn, oidQuery)) {
                routeDeleteRules(oidQuery.getOrElse(""))
              }
            })
          }
        } ~
          ...
          parameters("oid".?) { oidQuery =>
            authenticate()(authn => {
              entity(as[ExplainCreateReq]) { req =>
                req.rid match {
                  case Some(rid) =>
                    val oid = oidQuery.orElse(req.oid)
                    authorize(canAccessAdmin(authn, oidQuery)) {
                      complete(createRule(oid.getOrElse(""), rid, req))
                    }
                  case None =>
              ...
              entity(as[ExplainUpdateReq]) { req =>
                req.rid match {
                  case Some(rid) =>
                    val oid = oidQuery.orElse(req.oid)
                    authorize(canAccessAdmin(authn, oidQuery)) {
                      complete(updateRule(oid.getOrElse(""), rid, req))
                    }
                  case None =>
                    complete(StatusCodes.BadRequest -> "missing rid")
                }
      ...
      pathPrefix("search") {
        pathEndOrSingleSlash {
          authenticate()(authn =>
            authorize(canAccessAdmin(authn)) {
              post {
            ...
            parameters("oid".?) { oidQuery =>
              authenticate()(authn => {
                get {
                  ...
                  authorize(canAccessOid(authn, oidQuery)) {
                    complete(getRule(oidQuery.getOrElse(""), rid))
                  }
                } ~
                post {
                  entity(as[ExplainCreateReq]) { req =>
                    ...
                    authorize(canAccessOid(authn, oidQuery)) {
                      complete(createRule(oidQuery.getOrElse(""), rid, req))
                    }
                  }
                } ~
                put {
                  entity(as[ExplainUpdateReq]) { req =>
                    //val oid = oidQuery.orElse(req.oid).getOrElse(authOid(authn))
                    authorize(canAccessOid(authn, oidQuery)) {
                      complete(updateRule(oidQuery.getOrElse(""), rid, req))
                    }
                  }
                } ~
                delete {
                  //val oid = oidQuery.getOrElse(authOid(authn))
                  authorize(canAccessOid(authn, oidQuery)) {
                    complete(deleteRule(oidQuery.getOrElse(""), rid))
                  }
                }
              })
            }
