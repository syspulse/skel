...
class ChainlinkSpec extends AnyWordSpec with Matchers {
  ...
  "ChainlinkLoaderFeed" should {    
    ...
    "load feed from official Chainlink reference directory" in {
  ...
  "Chainlink Configuration" should {
    "load and verify Oracle configuration from application.conf" in {
      Chainlink.reset() // Start fresh
      
      val c = Configuration.withPriority(Seq(
        new ConfigurationAkka(from = Some("skel-crypto/eth-protocols/conf/application.conf"))      
      ))
      val config = c.getListString("contracts")
