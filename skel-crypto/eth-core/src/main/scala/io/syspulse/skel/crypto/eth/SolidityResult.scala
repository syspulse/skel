...
class SolidityResult {
  ...
  def parse(input0: String): Try[Val] = Try {
    ...
    // Check if input has multiple top-level expressions (comma-separated at top level)
    def hasMultipleTopLevelExpressions(s: String): Boolean = {
      var depth = 0
      ...
      for (c <- s) {
        c match {
          case '(' | '[' => depth += 1
          case ')' | ']' => depth -= 1
          case ',' if depth == 0 => foundComma = true
          case _ =>
        }
        if (foundComma && depth == 0) return true
      }
      false
    }
    
    val hasMultiple = hasMultipleTopLevelExpressions(input1)
    val simple = input1.startsWith("(") || input1.startsWith("[")
    val input = if(simple && !hasMultiple) input1 else s"(${input1})"
    val result = fastparse.parse(input, expr(_))
    result match {
      case fastparse.Parsed.Success(value0, _) =>
        val value = if(simple && !hasMultiple) value0 else RootVal(value0)
        value
