...
class Web3jTraceSpec extends AnyWordSpec with Matchers with BeforeAndAfterEach {
  ...
  // Test data
  // Use RPC endpoint from environment variable or system property (for providers that support debug_traceCall)
  // Examples: Alchemy (https://eth-mainnet.g.alchemy.com/v2/YOUR_API_KEY)
  //           Infura (https://mainnet.infura.io/v3/YOUR_API_KEY)
  //           QuickNode, GetBlock, dRPC, etc.
  // Set ETH_RPC_URL environment variable or -Deth.rpc.url system property to use an endpoint that supports debug methods
  // Usage: export ETH_RPC_URL="https://..." or sbt -Deth.rpc.url="https://..." "project eth_core" test
  val testRpcUrl = sys.env.get("ETH_RPC_URL")
    .orElse(sys.props.get("eth.rpc.url"))
    .getOrElse("https://eth.llamarpc.com")
  ...
  "Web3jTrace" should {
    ...
    "should handle different data formats" in {
      // Check if RPC supports debug_traceCall
      val checkResult = Try {
        web3jTrace.traceCall(testFrom, testTo, "0x").send()
      }
      
      if (checkResult.isFailure || (checkResult.isSuccess && checkResult.get.hasError() && 
          checkResult.get.getError() != null && 
          checkResult.get.getError().getMessage != null &&
          (checkResult.get.getError().getMessage.contains("not supported") ||
           checkResult.get.getError().getMessage.contains("does not exist")))) {
        cancel(s"RPC endpoint ($testRpcUrl) does not support debug_traceCall method. " +
               "Set ETH_RPC_URL environment variable or -Deth.rpc.url system property to an endpoint that supports debug methods " +
               "(e.g., Alchemy, Infura with API key, QuickNode, GetBlock, dRPC)")
      }
      ...
      testCases.foreach { case (data, description) =>
        ...
        if (response.hasError()) {
          cancel(s"RPC error: ${response.getError().getMessage()}")
        }
