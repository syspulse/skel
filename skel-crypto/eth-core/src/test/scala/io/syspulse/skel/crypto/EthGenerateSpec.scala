...
// [info]   at org.web3j.crypto.Keys.createEcKeyPair(Keys.java:82)
  "EthGenerate" should {
    "always generate 32bytes SK and 64bytes PK" ignore {
      for( i <- 1 to 1000) {
        val kk = Eth.generateRandom().get
        kk.sk.size should === (32)
        kk.pk.size should === (64)
      }
    }
    ...
    "generate for SK=0x1: PK=0x79be667ef9dcbbac55a06295ce870b07029bfcdb2dce28d959f2815b16f81798483ada7726a3c4655da4fbfc0e1108a8fd17b448a68554199c47d08ffb10d4b8" in {
      ...
      kk.sk.size should === (32)
      kk.pk.size should === (64)
      Eth.denormalize(kk.sk,kk.pk)
      Util.hex(kk.pk) should === ("0x79be667ef9dcbbac55a06295ce870b07029bfcdb2dce28d959f2815b16f81798483ada7726a3c4655da4fbfc0e1108a8fd17b448a68554199c47d08ffb10d4b8")
    }
    ...
    "generate for SK=0x2: PK=0xc6047f9441ed7d6d3045406e95c07cd85c778e4b8cef3ca7abac09b95c709ee51ae168fea63dc339a3c58419466ceaeef7f632653266d0e1236431a950cfe52a" in {
      val kk = Eth.generate("0x02").get
      kk.sk.size should === (32)
      kk.pk.size should === (64)
      Eth.denormalize(kk.sk,kk.pk)
      Util.hex(kk.pk) should === ("0xc6047f9441ed7d6d3045406e95c07cd85c778e4b8cef3ca7abac09b95c709ee51ae168fea63dc339a3c58419466ceaeef7f632653266d0e1236431a950cfe52a")
    }
    ...
    "generate for SK=0x3: PK=0xf9308a019258c31049344f85f89d5229b531c845836f99b08601f113bce036f9388f7b0f632de8140fe337e62a37f3566500a99934c2231b6cb9fd7584b8e672" in {
      val kk = Eth.generate("0x03").get
      kk.sk.size should === (32)
      kk.pk.size should === (64)
      Eth.denormalize(kk.sk,kk.pk)
      Util.hex(kk.pk) should === ("0xf9308a019258c31049344f85f89d5229b531c845836f99b08601f113bce036f9388f7b0f632de8140fe337e62a37f3566500a99934c2231b6cb9fd7584b8e672")
    }

    "generate multiple test keys with correct sizes (32,64)" in {
      for( i <- 0xff01 to 0xff05) {
        val kk = Eth.generate(Util.hex(BigInt(i).toByteArray)).get
        kk.sk.size should === (32)
        kk.pk.size should === (64)

        // correct denormalize
        val ec = Eth.denormalize(kk.sk,kk.pk)
      }
    }
  }
}
