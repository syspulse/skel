...
class WalletVaultSpec extends AnyWordSpec with Matchers with TestData {
  ...
  "WalletVaultTest" should {
    ...
    "sign and verify signature multisig" in {
      ...
      w1.mverify(sig,"message".getBytes(),Seq(PK(pk1)) ++ w1.signers.values.map(_.pk)) should === (4)
      w1.mverify(sig,"message1".getBytes(),Seq(PK(pk1)) ++ w1.signers.values.map(_.pk)) should === (0)
      w1.mverify(sig,"message".getBytes(),Seq(PK(pk2)) ++ w1.signers.values.map(_.pk)) should === (3)
    }

    "sign and verify signature multisig with arbitrary order" in {
      val w1 = new WalletVaultTest
      ...
      w1.mverify(sig,"message".getBytes(),w1.signers.values.map(_.pk).toSeq ++ Seq(PK(pk1))) should === (4)
      w1.mverify(sig,"message1".getBytes(),Seq(PK(pk1)) ++ w1.shuffle().signers.values.map(_.pk)) should === (0)
      w1.mverify(sig,"message".getBytes(),Seq(PK(pk2)) ++ w1.shuffle().signers.values.map(_.pk)) should === (3)
    }
  }
  ...
  "WalletVaultKeyfiles" should {
    ...
    "read keystores: only keystore-1.json" in {
      ...
      ss.toSeq(0)._2.addr === ("0x2b5ad5c4795c026514f8317c7a215e218dccd6cf")
    }
  ...
  "WalletVault" should {    
    ...
    "msign and mverfiy with WalletVaultTest + WalletVaultKeyfiles" in {
      ...
      w.mverifyAddress(sigs,"message".getBytes(),
      Seq(
        "0x2b5ad5c4795c026514f8317c7a215e218dccd6cf",
        "0x44c186f5335ea1e40a36c156b59602911a6b584d",
        "0xf72e843da09a929dc71c9224a52da0c2660053dc"
      ) ++ w.signers.map(_.addr)
      ) should === (true)

      w.mverifyAddress(sigs,"message1".getBytes(),
        Seq(
          "0x2b5ad5c4795c026514f8317c7a215e218dccd6cf",
          "0x44c186f5335ea1e40a36c156b59602911a6b584d",
          "0xf72e843da09a929dc71c9224a52da0c2660053dc"
    ...
    //   w6.mverifyAddress(sigs3,"message".getBytes(), m = 3) should === (true)
    //   w6.mverifyAddress(sigs3,"message1".getBytes(), m = 3) should === (false)  
    ...
    //   w61.mverifyAddress(sigs3,"message".getBytes(), m = 3) should === (true)
    //   w61.mverifyAddress(sigs3,"message1".getBytes(), m = 3) should === (false)  
