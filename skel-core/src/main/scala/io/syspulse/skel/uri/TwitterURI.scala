...
twitter://consumer_key:consumer_secret/id,id?past=1&var=1
*/
case class TwitterURI(uri:String) {
  val PREFIX = "twitter://"

  private val (_consumerKey:String,_consumerSecret:String,_accessKey:String,_accessSecret:String,_follow:Seq[String],_past:Int,_ops:Map[String,String]) 
    = parse(uri)
  ...
  def parse(uri:String):(String,String,String,String,Seq[String],Int,Map[String,String]) = {
    // resolve options
    ...
    url.stripPrefix(PREFIX).split("[:/]").toList match {
      case consumerKey :: consumerSecret :: follow :: Nil => 
        ( Util.replaceEnvVar(consumerKey),Util.replaceEnvVar(consumerSecret),
          "","",
          follow.split(",").toSeq,
          ops.get("past").map(_.toInt).getOrElse(1),
          ops
        )
