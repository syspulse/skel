...
class TelURISpec extends AnyWordSpec with Matchers {
  ...
  "TelURI" should {
    ...
    "parse tel://phone (digits only)" in {
      ...
      u.sessionPath shouldBe Some("1234567890.session")
    ...
    "parse tel://phone?session=path" in {
      ...
      u.phone shouldBe Some("1234567890")
      u.sessionPath shouldBe Some("/tmp/my.session")
    }

    "parse tel:// with api_id, api_hash, phone in query" in {
      val u = TelURI("tel://?api_id=12345&api_hash=abcdef&phone=+79991234567")
      u.apiId shouldBe Some(12345)
      u.apiHash shouldBe Some("abcdef")
      u.phone shouldBe Some("+79991234567")
    }
    ...
    "use default session path from phone when session not in URI" in {
      ...
      u.sessionPath shouldBe Some("15551234567.session")
    }
    ...
    "ignore non-phone path (phone from query or env only)" in {
