...
trait Agent extends PollingHelper {
  ...
  def processRun(run: Run, thread: Thread): Future[Run] = {
    ...
    if(run.status == RunStatus.InProgress) {
      // keep polling
      log.info(s"${getId()}: waiting for state change...")
    ...
    log.info(s"${getId()}: Processing Run: toolCalls=${toolCalls}")
            ...
            .map(r => {
              ...
              r match {
                case Some(run) => 
                  ...
                  run
                case None => throw new IllegalStateException(s"${getId()}: Run ${run0.id}: not found")
              }
            })
        }        
