...
echo "  sshpass -p '${PASSWORD}' ssh -D 127.0.0.1:1090 -N -f -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o PasswordAuthentication=yes -p 2222 root@localhost"
echo ""
echo "Or interactively (keeps connection open, will prompt for password):"
echo "  ssh -D 127.0.0.1:1090 -N -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o PasswordAuthentication=yes -p 2222 root@localhost"
echo ""
echo "Or using sshpass for automated password (if installed):"
echo "  sshpass -p '${PASSWORD}' ssh -D 127.0.0.1:1090 -N -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o PasswordAuthentication=yes -p 2222 root@localhost"
echo ""
